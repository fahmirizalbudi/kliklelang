...
@section('title', 'KlikLelang - Edit Barang')

@section('content')
  <div class="edit-barang">
    <x-breadcrumb groupPage="Barang" currentPage="Edit Barang"></x-breadcrumb>
    <section>
      <form action="{{ route('barang.update', $barang) }}" method="POST" class="barang-form" enctype="multipart/form-data">
        @csrf
        @method('PUT')
        <x-form-card title="Informasi Barang">
          <div class="barang-info-form">
            <div>
              ...
              <x-text-field placeholder="Masukkan nama barang" type="text" name="nama_barang" defaultValue="{{ $barang->nama_barang }}"></x-text-field>
            </div>
            <div>
              <x-label-field forField="harga_awal">Harga Awal</x-label-field>
              <x-text-field placeholder="12000" type="number" name="harga_awal"
                defaultValue="{{ $barang->harga_awal }}"></x-text-field>
            </div>
            <div class="deskripsi-barang-group">
              ...
              <textarea name="deskripsi_barang" rows="7" id="deskripsi-barang"
                placeholder="Deskripsikan barang ini dengan detail"
        ...
        <x-form-card title="Foto Barang">
          <label for="foto-barang" class="upload-foto-barang">
            ...
            <input type="file" name="foto_barang" id="foto-barang"
              style="opacity: 0; position: absolute; top: 45%; left: 46%">
          </label>
        </x-form-card>
        <div class="barang-form-actions">
          <button class="submit-button" type="submit">Perbarui Barang</button>
        </div>
      </form>
    </section>
  </div>
